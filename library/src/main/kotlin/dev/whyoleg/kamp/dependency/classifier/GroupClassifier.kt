...
fun GroupClassifier.dependency(name: String, version: String, provider: DependencyProvider, target: TargetWithPostfix<*>): MultiDependency =
    raw(name, version, provider)(target)

fun GroupClassifier.dependency(name: String, version: String, provider: DependencyProvider, targets: Set<TargetWithPostfix<*>>): MultiDependency =
    raw(name, version, provider)(targets)

fun GroupClassifier.dependency(name: String, version: String, provider: DependencyProvider, vararg targets: TargetWithPostfix<*>): MultiDependency =
    raw(name, version, provider)(*targets)

fun <C> C.dependency(name: String, version: String, provider: DependencyProvider): MultiDependency where C : GroupClassifier, C : MultiTargetClassifier =
    raw(name, version, provider)(targets)
